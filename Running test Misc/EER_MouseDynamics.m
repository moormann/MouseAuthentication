...
while ~feof(imposId)
 ...
 nameF = fopen('Names.txt');
 nameCount = 0;
 avgEER = 0;
 fileName = fgetl(nameF);
 
 while ~feof(nameF)
 
 nameCount = nameCount + 1;
     
 false_scoreS_750=load(strcat(strcat(imposName,fileName),'Impos.txt'));
 true_scoresS_750=load(strcat(strcat(imposName,fileName),'Legit.txt'));
...
avgEER = avgEER + EER_750;
 fileName = fgetl(nameF);

 end

 avgEER = avgEER / nameCount;
...
elseif secLowest > avgEER
    thiLowest = secLowest;
    secLowest = avgEER;
    
    thiLowestFN = secLowestFN;
    secLowestFN = imposName;
elseif thiLowest > avgEER
    thiLowest = avgEER;
